        ...
        .then(async (res) => {
          ...
          if (json) {
            ...
            if (Table.AttributeDefinitions)
              setSchema(Table.AttributeDefinitions);
    ...
    fetch('/api/dynamodb/delete_item', {
      ...
      .then(async (response) => {
        if (response.status === 200) {
          ...
          setRows(rows.filter((row) => JSON.stringify(row) !== string_item));
        } else
    ...
    <>
      ...
      >
        ...
        >
          ...
          {schema.map(({ AttributeName, AttributeType }) => (
            <Form.Item
              key={AttributeName}
              name={AttributeName}
              label={AttributeName}
            ...
            >
              {AttributeType === 'S' ? (
                <Input placeholder={`Give an ${AttributeName}`} />
              ) : (
                <InputNumber
