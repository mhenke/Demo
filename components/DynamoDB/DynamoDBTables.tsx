...
const DynamoDBTables: React.FC = () => {
  ...
  const onFinish = async ({ TableName, KeySchema_data }: FormTableType) => {
    ...
    const KeySchema = KeySchema_data.map(({ AttributeName }) => ({
      AttributeName,
      KeyType: 'HASH',
    }));
  ...
  useEffect(() => {
    fetch('api/dynamodb/all_tables', { method: 'GET' }).then((res) => {
      res.json().then((json) => {
        setTables(json);
      });
    });
  ...
  return (
    <>
      ...
      >
        ...
        >
          ...
          <Title type='warning' level={4}>
            {
              "Key attributions can't be zero or  exceed 2 and we are only dealing with KeyAttributes for this demo"
            }
          </Title>
                  ...
                  <Row key={key} style={{ background: 'white' }}>
                    <Col span={11}>
                      <Form.Item
                        ...
                        rules={[
                          { required: true, message: 'Missing AttributeName' },
                        ]}
                      >
                    ...
                    <Col span={11}>
                      <Form.Item
                        ...
                        rules={[
                          { required: true, message: 'Missing AttributeType' },
                        ]}
                      >
      ...
      <List
        ...
        dataSource={tables}
        renderItem={(
          { TableName, AttributeDefinitions, CreationDateTime },
          index
        ) => (
          <List.Item>
            <Card
